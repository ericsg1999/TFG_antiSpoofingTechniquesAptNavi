...
%       acqMode       - 'Normal': Acquisition in normal mode will look for the presence of all 
%                                 possible satellitesin the received signal. This mode is used 
%                                 in the initial acquisition, when the receiver has still not 
%                                 acquired any satellite, therefore still does not know which
%                                 satellites are present. The objective of this acquisition mode 
%                                 is to determine the visible satellites and provide an estimation 
%                                 of the Doppler frequency and code phase parameters.
%                     - 'APT': Acquisition in APT mode will only perform the acquisition of the previously 
%                              acquired satellites, which will be indicated in the SatellitePresentList 
%                              input parameter. In this mode, the acquisition function does not look for 
%                              satellites that were declared as not visible in the initial acquisition. 
%                              By restricting the number of satellites to acquire, the computational load 
%                              is remarkable reduced as it is assumed that the spoofer will only perform the 
%                              attack to the satellites that are visible for the victim receiver.

%      SatellitePresentList - In case of acqMode=APT, the function needs to know the present satellites in order
%                             to only perform the acquisition in those satellites. 
